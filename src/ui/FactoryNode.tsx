...
export const FactoryNodeView = ({ data, selected }: NodeProps<FactoryFlowNode>) => {
  ...
  return <article className={`graph-node graph-node--${node.kind}${selected ? ' is-selected' : ''}${data.issueSeverity !== undefined ? ` has-${data.issueSeverity}` : ''}`} style={{ width: gridToPixel(node.footprint.width), height: gridToPixel(node.footprint.height) }} aria-label={`${node.kind}: ${node.name}${data.diagnostic === undefined ? '' : `. ${data.diagnostic}`}`}>
    ...
    {hasActivityMeter(node.kind) && <div className="activity"><span style={{ width: `${data.activity ?? 0}%` }} /><small>{Math.round(data.activity ?? 0)}%</small></div>}
    <div className="ports">
      {node.ports.map((port, portIndex) => {
        ...
        const craftFlow = nominalPortRate(node, portIndex, data.childContract)
        ...
        const flowState = flow === undefined ? 'pending' : flow === 0n ? 'idle' : flow >= meterLimit ? 'saturated' : 'active'
        const limitDescription = craftFlow === undefined ? `connector capacity ${formatRate(port.capacity)}/s` : `craft rate ${formatRate(craftFlow)}/s; connector capacity ${formatRate(port.capacity)}/s`
        const rateDescription = flow === undefined ? `Flow unavailable; ${limitDescription}` : `Flow ${displayedFlow}; ${limitDescription}; utilization ${utilization ?? 0}%`
        ...
        return <div className={`port-row port-row--${port.direction} is-${flowState}`} key={port.id} style={{ color: resource?.colour, top: top - 9 }}>
          ...
          <small className="port-rate" title={rateDescription} aria-label={`Flow details for ${resourceName} (${port.direction}): ${rateDescription}`}>
