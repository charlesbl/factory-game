...
const keepMostSevere = <Id>(
  map: Map<Id, CompileDiagnostic>,
  id: Id,
  ...
  if (
    current === undefined ||
    severityRank[diagnostic.severity] > severityRank[current.severity]
  )
    map.set(id, diagnostic);
};

export const buildDiagnosticOverlay = (
  ...
  for (const diagnostic of diagnostics) {
    ...
    if (edgeId !== undefined)
      keepMostSevere(edgeDiagnostics, edgeId, diagnostic);
    if (diagnostic.details?.otherEdgeId !== undefined)
      keepMostSevere(
        edgeDiagnostics,
        asId<EdgeId>(diagnostic.details.otherEdgeId),
        diagnostic,
      );
    if (nodeId !== undefined)
      keepMostSevere(nodeDiagnostics, nodeId, diagnostic);
    for (const edge of blueprint.edges.values()) {
      const touchesPort =
        portId !== undefined &&
        (edge.sourcePortId === portId || edge.targetPortId === portId);
      const entersLimitedNode =
        diagnostic.code === 'LIMITED_INPUT' &&
        edge.targetNodeId === nodeId &&
        (resourceId === undefined ||
          routeResource(blueprint, edge) === resourceId);
      const leavesLimitedNode =
        diagnostic.code === 'LIMITED_OUTPUT' &&
        edge.sourceNodeId === nodeId &&
        (resourceId === undefined ||
          routeResource(blueprint, edge) === resourceId);
      const touchesBrokenNode =
        (diagnostic.code === 'CYCLE' ||
          diagnostic.code === 'INTERNAL_VERIFICATION') &&
        nodeId !== undefined &&
  ...
  if (diagnostics.some((diagnostic) => diagnostic.severity === 'error'))
    for (const edgeId of blueprint.edges.keys())
      if (!edgeDiagnostics.has(edgeId)) blockedEdges.add(edgeId);
