...
export const FactoryGraphEditor = ({ blueprint, contract, childContracts, diagnostics, diagnosticsVisible, selection: controlledSelection, placement, onCommand, onSelection, onPlacementCommit, onPlacementCancel }: Props) => {
  ...
  const placementBlueprint = useMemo(() => {
    ...
    return { ...blueprint, nodes: new Map([...blueprint.nodes, ...placementSubgraph.nodes.map((node) => [node.id, node] as const)]), edges: new Map([...blueprint.edges, ...placementSubgraph.edges.map((edge) => [edge.id, edge] as const)]), externalPorts: [...blueprint.externalPorts, ...placementSubgraph.externalPorts] }
  ...
  const reportSelection = useCallback((nodeIds: readonly NodeId[], edgeIds: readonly EdgeId[]) => {
    if (controlledSelection.handle === undefined && sameIds(nodeIds, controlledSelection.nodeIds) && sameIds(edgeIds, controlledSelection.edgeIds)) return
    setSelectedHandle(undefined); onSelection({ nodeIds, edgeIds })
