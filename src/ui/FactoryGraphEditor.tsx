import { Background, BackgroundVariant, Controls, MarkerType, MiniMap, ReactFlow, SelectionMode, type EdgeChange, type NodeChange, type ReactFlowInstance, type Viewport, applyEdgeChanges, applyNodeChanges } from '@xyflow/react'
...
import type { FactoryBlueprint, LooseConnection, MaterializedSubgraph, PortReference, RouteHandle } from '../editor'
...
export interface GraphSelection { readonly nodeIds: readonly NodeId[]; readonly edgeIds: readonly EdgeId[]; readonly looseConnectionIds: readonly LooseConnectionId[]; readonly handle?: HandleSelection }
export interface GraphPlacement { readonly label: string; readonly subgraph: MaterializedSubgraph }
...
interface Props { readonly blueprint: FactoryBlueprint; readonly contract: FactoryContract | undefined; readonly childContracts: ReadonlyMap<string, FactoryContract>; readonly diagnostics: readonly CompileDiagnostic[]; readonly diagnosticsVisible: boolean; readonly selection: GraphSelection; readonly placement?: GraphPlacement; readonly onCommand: (command: EditCommand) => void; readonly onSelection: (selection: GraphSelection) => void; readonly onPlacementCommit: (position: GridPoint, repeat: boolean) => void; readonly onPlacementCancel: () => void }
...
export const FactoryGraphEditor = ({ blueprint, contract, childContracts, diagnostics, diagnosticsVisible, selection: controlledSelection, placement, onCommand, onSelection, onPlacementCommit, onPlacementCancel }: Props) => {
  ...
  const selectedNodeIds = useMemo(() => new Set(controlledSelection.nodeIds), [controlledSelection.nodeIds])
  const selectedEdgeIds = useMemo(() => new Set(controlledSelection.edgeIds), [controlledSelection.edgeIds])
  const selectedLooseConnectionIds = useMemo(() => new Set(controlledSelection.looseConnectionIds), [controlledSelection.looseConnectionIds])
  ...
  const placementBlueprint = useMemo(() => {
    ...
    return { ...blueprint, nodes: new Map([...blueprint.nodes, ...placementSubgraph.nodes.map((node) => [node.id, node] as const)]), edges: new Map([...blueprint.edges, ...placementSubgraph.edges.map((edge) => [edge.id, edge] as const)]), looseConnections: new Map([...blueprint.looseConnections, ...placementSubgraph.looseConnections.map((loose) => [loose.id, loose] as const)]), externalPorts: [...blueprint.externalPorts, ...placementSubgraph.externalPorts] }
  ...
  const movePlacementGhost = (position: GridPoint) => {
    graph.current?.style.setProperty('--placement-x', `${gridToPixel(position.x)}px`)
    graph.current?.style.setProperty('--placement-y', `${gridToPixel(position.y)}px`)
  }
  ...
  const projectedNodes = useMemo<FactoryFlowNode[]>(() => {
    ...
    return { id: node.id, type: 'factory' as const, position: { x: gridToPixel(node.position.x), y: gridToPixel(node.position.y) }, data: { node, portResources, onPortPointerDown: beginPortDrag, ...(childContract === undefined ? {} : { childContract }), ...(portFlows !== undefined ? { portFlows } : {}), activity: Number(contract?.machineActivity.get(node.id) ?? 0n) / 10_000, ...(diagnostic !== undefined ? { diagnostic: diagnosticText(diagnostic), issueSeverity: diagnostic.severity } : {}) }, draggable: placement === undefined, selected: placement === undefined && selectedNodeIds.has(node.id) }
    })
    const ghost = placementSubgraph?.nodes.map((node): FactoryFlowNode => {
      const childContract = node.kind === 'sub-factory' ? childContracts.get(node.contractId) : undefined
      const portResources = new Map(node.ports.flatMap((port) => 'resourceId' in port && port.resourceId !== undefined ? [[port.id, port.resourceId] as const] : []))
      return { id: node.id, type: 'factory', position: { x: gridToPixel(node.position.x), y: gridToPixel(node.position.y) }, data: { node, portResources, ...(childContract === undefined ? {} : { childContract }) }, draggable: false, selectable: false, focusable: false, className: 'placement-flow-node', zIndex: 20 }
    }) ?? []
    return [...committed, ...ghost]
  }, [beginPortDrag, blueprint, childContracts, contract, diagnosticOverlay, placement, placementSubgraph, portFlows, selectedNodeIds])
  const projectedEdges = useMemo<ConveyorFlowEdge[]>(() => {
    ...
    return { id: edge.id, type: 'conveyor' as const, source: edge.sourceNodeId, sourceHandle: edge.sourcePortId, target: edge.targetNodeId, targetHandle: edge.targetPortId, data: { sections, transitions, width: diagnostic !== undefined ? 5 : 2 + Math.min(5, ratio / 25), invalid: diagnostic?.severity === 'error' || blocked, saturated: ratio >= 95 }, animated: flowRate > 0n && diagnostic === undefined && !blocked, selected: placement === undefined && selectedEdgeIds.has(edge.id), style: { stroke: colour, opacity: diagnostic !== undefined ? 1 : blocked ? .55 : flowRate === 0n ? .35 : 1, ...(blocked ? { strokeDasharray: '8 7' } : {}) }, markerEnd: { type: MarkerType.ArrowClosed, color: colour }, zIndex: diagnostic !== undefined ? 10 : 1, ...(diagnostic !== undefined ? { className: `diagnostic-edge diagnostic-edge--${diagnostic.severity}` } : blocked ? { className: 'diagnostic-edge diagnostic-edge--blocked' } : {}) }
    })
    const ghost = placementSubgraph?.edges.map((edge): ConveyorFlowEdge => {
      const source = placementBlueprint ?? blueprint
      const colour = routeResource(source, edge) === undefined ? '#8ba39a' : resourceById.get(routeResource(source, edge)!)?.colour ?? '#8ba39a'
      const sections = routeSections(source, edge).map((section) => ({ layerId: section.layerId, points: section.points.map((point) => ({ x: gridToPixel(point.x), y: gridToPixel(point.y) })) })); const transitions = routeBridgeMarkers(source, edge).map((point) => ({ x: gridToPixel(point.x), y: gridToPixel(point.y) }))
      return { id: edge.id, type: 'conveyor', source: edge.sourceNodeId, sourceHandle: edge.sourcePortId, target: edge.targetNodeId, targetHandle: edge.targetPortId, data: { sections, transitions, width: 3 }, animated: false, selectable: false, focusable: false, className: 'placement-flow-edge', style: { stroke: colour }, markerEnd: { type: MarkerType.ArrowClosed, color: colour }, zIndex: 19 }
    }) ?? []
    return [...committed, ...ghost]
  }, [blueprint, contract, diagnosticOverlay, displayBlueprint, placement, placementBlueprint, placementSubgraph, selectedEdgeIds])
  ...
  const onNodesChange = useCallback((changes: NodeChange<FactoryFlowNode>[]) => {
    ...
    const removed: EditCommand[] = []; const moved: EditCommand[] = []
    for (const change of changes) {
      if (change.type === 'remove' && blueprint.nodes.has(asId<NodeId>(change.id))) removed.push(removeNode(asId<NodeId>(change.id)))
      if (change.type === 'position' && change.dragging === false && change.position !== undefined && blueprint.nodes.has(asId<NodeId>(change.id))) moved.push(moveNode(asId<NodeId>(change.id), gridPoint(pixelToGrid(change.position.x), pixelToGrid(change.position.y))))
    }
    if (removed.length > 0) onCommand(transaction('Delete nodes', removed))
    if (moved.length > 0) onCommand(transaction(moved.length === 1 ? 'Move node' : 'Move selection', moved))
  ...
  const reportSelection = useCallback((nodeIds: readonly NodeId[], edgeIds: readonly EdgeId[], looseConnectionIds: readonly LooseConnectionId[] = []) => {
    if (controlledSelection.handle === undefined && sameIds(nodeIds, controlledSelection.nodeIds) && sameIds(edgeIds, controlledSelection.edgeIds) && sameIds(looseConnectionIds, controlledSelection.looseConnectionIds)) return
    setSelectedHandle(undefined); onSelection({ nodeIds, edgeIds, looseConnectionIds })
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') { setBridgeMenu(undefined); if (placement !== undefined) { event.preventDefault(); event.stopImmediatePropagation(); onPlacementCancel(); return } if (connectionDrag !== undefined) { event.preventDefault(); setConnectionDrag(undefined); setPointer(undefined); return } }
      if ((event.key !== 'Delete' && event.key !== 'Backspace') || selectedHandle === undefined) return
      event.preventDefault(); event.stopImmediatePropagation(); if (selectedHandle.edgeId !== undefined) onCommand(removeRouteHandle(selectedHandle.edgeId, selectedHandle.handleId)); else if (selectedHandle.looseId !== undefined) onCommand(removeLooseHandle(selectedHandle.looseId, selectedHandle.handleId)); setSelectedHandle(undefined); onSelection({ nodeIds: [], edgeIds: [], looseConnectionIds: [] })
    }
  ...
  const beginHandleMove = (event: React.PointerEvent<HTMLButtonElement>, value: HandleSelection, position: GridPoint) => { event.preventDefault(); event.stopPropagation(); event.currentTarget.setPointerCapture(event.pointerId); setSelectedHandle(value); setHandleDrag({ ...value, start: position }); setPointer(position); onSelection({ nodeIds: [], edgeIds: value.edgeId === undefined ? [] : [value.edgeId], looseConnectionIds: value.looseId === undefined ? [] : [value.looseId], handle: value }) }
  const deleteHandle = (event: React.MouseEvent<HTMLButtonElement>, value: HandleSelection) => { event.preventDefault(); event.stopPropagation(); setHandleDrag(undefined); setPointer(undefined); setSelectedHandle(undefined); if (value.edgeId !== undefined) onCommand(removeRouteHandle(value.edgeId, value.handleId)); else if (value.looseId !== undefined) onCommand(removeLooseHandle(value.looseId, value.handleId)); onSelection({ nodeIds: [], edgeIds: value.edgeId === undefined ? [] : [value.edgeId], looseConnectionIds: value.looseId === undefined ? [] : [value.looseId] }) }
  ...
  return <div ref={graph} className={`graph-canvas${placement === undefined ? '' : ' is-placing'}`} aria-label="Factory graph editor" onClickCapture={(event) => { if (placement === undefined) return; const position = pointFromEvent(event); if (position === undefined) return; event.preventDefault(); event.stopPropagation(); onPlacementCommit(position, event.shiftKey) }} onPointerMoveCapture={(event) => { const next = pointFromEvent(event); if (next !== undefined) { movePlacementGhost(next); if (connectionDrag !== undefined || handleDrag !== undefined) setPointer(next) } }} onPointerUpCapture={(event) => {
    ...
    <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} edgeTypes={edgeTypes} onInit={(instance) => { flow.current = instance; setViewport(instance.getViewport()) }} onMove={(_, nextViewport) => setViewport(nextViewport)} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeClick={(event, node) => { if (placement !== undefined) return; const id = asId<NodeId>(node.id); if (event.ctrlKey || event.metaKey) { const next = new Set(controlledSelection.nodeIds); if (next.has(id)) next.delete(id); else next.add(id); reportSelection([...next], controlledSelection.edgeIds, controlledSelection.looseConnectionIds) } else reportSelection([id], []) }} onEdgeClick={(event, edge) => { if (placement !== undefined) return; const id = asId<EdgeId>(edge.id); if (event.ctrlKey || event.metaKey) { const next = new Set(controlledSelection.edgeIds); if (next.has(id)) next.delete(id); else next.add(id); reportSelection(controlledSelection.nodeIds, [...next], controlledSelection.looseConnectionIds) } else reportSelection([], [id]) }} onPaneClick={() => { if (placement === undefined) reportSelection([], []) }} onSelectionEnd={() => { const instance = flow.current; if (instance === null || placement !== undefined) return; reportSelection(instance.getNodes().filter((node) => node.selected && blueprint.nodes.has(asId<NodeId>(node.id))).map((node) => asId<NodeId>(node.id)), instance.getEdges().filter((edge) => edge.selected && blueprint.edges.has(asId<EdgeId>(edge.id))).map((edge) => asId<EdgeId>(edge.id))) }} fitView snapToGrid snapGrid={[GRID_SIZE, GRID_SIZE]} deleteKeyCode={null} selectionOnDrag selectionMode={SelectionMode.Partial} panOnDrag={[1]} panActivationKeyCode="Space" minZoom={.25} maxZoom={1.7} nodesDraggable={placement === undefined} nodesConnectable={false} elementsSelectable={placement === undefined}>
    ...
    <div className="route-overlay" aria-label="Route handles">
      <svg aria-hidden="true">{looseRoutes.map(({ loose, route }) => <polyline className={`loose-route${selectedLooseConnectionIds.has(loose.id) ? ' is-selected' : ''}`} key={loose.id} points={route.points.map((point) => `${screenPoint(point).x},${screenPoint(point).y}`).join(' ')} />)}{preview !== undefined && <polyline className="route-preview" points={preview.points.map((point) => `${screenPoint(point).x},${screenPoint(point).y}`).join(' ')} />}</svg>
      {[...blueprint.edges.values()].flatMap((edge) => edgeRoute(blueprint, edge).points.slice(1).map((end, index) => ({ edge, start: edgeRoute(blueprint, edge).points[index]!, end, index }))).map(({ edge, start, end, index }) => {
        ...
        return <button key={`hit-${edge.id}-${index}`} className="route-segment-hit" data-edge-id={edge.id} aria-label={`Select route segment ${index + 1}`} style={style} onClick={(event) => { event.stopPropagation(); setSelectedHandle(undefined); setBridgeMenu(undefined); onSelection({ nodeIds: [], edgeIds: [edge.id], looseConnectionIds: [] }) }} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); const position = pointFromEvent(event); if (position !== undefined) setBridgeMenu({ edgeId: edge.id, position }) }} onDoubleClick={(event) => { event.preventDefault(); event.stopPropagation(); const point = pointFromEvent(event); if (point === undefined) return; const handle = { id: asId<RouteHandleId>(`handle-${crypto.randomUUID()}`), position: point }; onCommand(insertRouteHandle(edge.id, nearestRouteSpan(edgeRoute(blueprint, edge), point), handle)); const next = { edgeId: edge.id, handleId: handle.id }; setSelectedHandle(next); onSelection({ nodeIds: [], edgeIds: [edge.id], looseConnectionIds: [], handle: next }) }} />
      })}
      {looseRoutes.flatMap(({ loose, route }) => route.points.slice(1).map((end, index) => ({ loose, start: route.points[index]!, end, index }))).map(({ loose, start, end, index }) => {
        const a = screenPoint(start); const b = screenPoint(end); const horizontal = a.y === b.y; const length = Math.abs(horizontal ? b.x - a.x : b.y - a.y); if (length <= 16) return null
        const style = horizontal ? { left: Math.min(a.x, b.x) + 8, top: a.y - 9, width: length - 16, height: 18 } : { left: a.x - 9, top: Math.min(a.y, b.y) + 8, width: 18, height: length - 16 }
        return <button key={`loose-hit-${loose.id}-${index}`} className="route-segment-hit loose-route-segment-hit" data-loose-id={loose.id} aria-label={`Select incomplete route segment ${index + 1}`} style={style} onClick={(event) => { event.stopPropagation(); setSelectedHandle(undefined); setBridgeMenu(undefined); if (event.ctrlKey || event.metaKey) { const next = new Set(controlledSelection.looseConnectionIds); if (next.has(loose.id)) next.delete(loose.id); else next.add(loose.id); reportSelection(controlledSelection.nodeIds, controlledSelection.edgeIds, [...next]) } else reportSelection([], [], [loose.id]) }} />
      })}
      ...
      {placement !== undefined && <div className="placement-hint" role="status" aria-live="polite">Place {placement.label} · click to confirm · Shift+click to repeat · Esc to cancel</div>}
