...
export interface ConveyorSectionView {
...
const pathFor = (points: ConveyorSectionView['points']): string =>
  points.length === 0
    ? ''
    : `M ${points.map((point) => `${point.x} ${point.y}`).join(' L ')}`;

export const ConveyorEdgeView = ({
  id,
  data,
  markerEnd,
  selected,
  style,
}: EdgeProps<ConveyorFlowEdge>) => {
    ...
    <>
      {paths.map((section, index) => (
        <BaseEdge
          ...
          {...(index === paths.length - 1 && markerEnd !== undefined
            ? { markerEnd }
            : {})}
      ...
      {data.transitions?.map((point, index) => (
        <g
        ...
        >
          <circle r="7" />
          <path d="M -5 0 L 5 0 M -2 -3 L 2 0 L -2 3" />
        </g>
      ))}
      {data.drcMarkers?.map((point, index) => (
        <g
