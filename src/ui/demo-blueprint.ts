import { asId, gridPoint, parseRate, recipeById, resourceById } from '../domain'
...
export const createDemoBlueprint = (): FactoryBlueprint => {
  ...
  return { id: asId<FactoryId>('factory-main'), revision: 0, nodes: new Map<NodeId, BlueprintNode>([[input.id, input], [furnace.id, furnace], [output.id, output]]), edges: new Map([[oreEdge.id, oreEdge], [ingotEdge.id, ingotEdge]]), looseConnections: new Map(), externalPorts: [{ nodeId: input.id, portId: input.ports[0]!.id, side: 'left', offset: 1 }, { nodeId: output.id, portId: output.ports[0]!.id, side: 'right', offset: 1 }] }
}
...
export const createBoundaryNode = (id: NodeId, portId: PortId, kind: 'external-input' | 'external-output', resourceId: ResourceId, x: number, y: number, capacity = '6'): BlueprintNode => {
  const output = kind === 'external-input'; const resource = resourceById.get(resourceId)
  return { id, kind, name: `${resource?.name ?? resourceId} ${output ? 'intake' : 'dispatch'}`, position: gridPoint(x, y), footprint: rect(4, 2), ports: [port(portId, output ? 'output' : 'input', resourceId, capacity, output ? 4 : 0, 1)] }
}
