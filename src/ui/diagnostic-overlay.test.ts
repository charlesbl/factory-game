...
describe('diagnostic overlay', () => {
  it('highlights every route connected to an over-subscribed port', () => {
    ...
    const secondEdge = {
      ...originalEdge,
      id: asId<EdgeId>('edge-ingot-2'),
      sourceNodeId: asId<NodeId>('node-second-furnace'),
    };
    const blueprint = {
      ...original,
      edges: new Map(original.edges).set(secondEdge.id, secondEdge),
    };
    const diagnostic: CompileDiagnostic = {
      code: 'PORT_OCCUPIED',
      severity: 'error',
      entity: {
        nodeId: asId<NodeId>('node-iron-output'),
        portId: asId<PortId>('port-ingot-target'),
      },
    };

    const overlay = buildDiagnosticOverlay(blueprint, [diagnostic]);

    expect([...overlay.edgeDiagnostics.keys()].sort()).toEqual([
      'edge-ingot',
      'edge-ingot-2',
    ]);
  ...
  it('marks unrelated routes as blocked only for errors', () => {
    ...
    const warning: CompileDiagnostic = {
      ...
      severity: 'warning',
      entity: {
        nodeId: asId<NodeId>('node-furnace'),
        resourceId: asId('ironIngot'),
      },
    };
