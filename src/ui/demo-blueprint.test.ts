...
describe('starter factory', () => {
  it('uses the same constructors as player-placed machines and boundary ports', () => {
    ...
    expect(blueprint.nodes.get(asId<NodeId>('node-furnace'))).toEqual(
      createMachineNode(
    ...
    expect(blueprint.nodes.get(asId<NodeId>('node-iron-input'))).toEqual(
      createBoundaryNode(
    ...
    expect(blueprint.nodes.get(asId<NodeId>('node-iron-output'))).toEqual(
      createBoundaryNode(
    ...
    expect(
      blueprint.nodes.get(asId<NodeId>('node-iron-input'))?.footprint.width,
    ).toBe(4);
    expect(
      blueprint.nodes.get(asId<NodeId>('node-iron-input'))?.ports[0]?.anchor.x,
    ).toBe(4);
  });
});
