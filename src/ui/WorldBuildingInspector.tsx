...
const resourceName = (id: ResourceId): string =>
  resourceById.get(id)?.name ?? id;
const percent = (quantity: number, capacity: number): number =>
  capacity === 0 ? 0 : Math.min(100, Math.max(0, (quantity / capacity) * 100));
...
const Meter = ({
  ...
  <span className={`world-inventory-meter ${className}`} aria-hidden="true">
...
const Inventory = ({
  ...
  return (
    <div className="world-inventory">
      <div className="world-inventory-heading">
        <strong>{title}</strong>
        <span>
          {total} / {inventory.capacity}
        </span>
      </div>
      <Meter value={percent(total, inventory.capacity)} />
      {inventory.items.length === 0 ? (
        <p className="world-empty-state">Empty</p>
      ) : (
        <ul>
          {inventory.items.map((item) => (
            <li key={item.resourceId}>
              <span
                className="world-resource-dot"
                style={{
                  background: resourceById.get(item.resourceId)?.colour,
                }}
              />
              <span>{resourceName(item.resourceId)}</span>
              <b>{item.quantity}</b>
            </li>
          ))}
        </ul>
...
const FactoryState = ({
  ...
  readonly state: Extract<WorldBuildingSnapshot, { kind: 'factory' }>;
}) => (
  <>
    <div className="world-building-status">
    ...
    <div className="world-buffer-group">
      ...
      {state.inputs.length === 0 ? (
        <p className="world-empty-state">No inputs</p>
      ) : (
        state.inputs.map((buffer) => (
          <div className="world-buffer" key={buffer.resourceId}>
            <div>
              <span>{resourceName(buffer.resourceId)}</span>
              <b>
                {buffer.quantity} / {buffer.capacity}
              </b>
            </div>
            <Meter value={percent(buffer.quantity, buffer.capacity)} />
            <small>
              Internal WIP <b>{buffer.wipPercent.toFixed(1)}%</b>
            </small>
            <Meter value={buffer.wipPercent} className="is-wip" />
          </div>
        ))
      )}
    </div>
    <div className="world-buffer-group">
      <h4>Output buffers</h4>
      {state.outputs.length === 0 ? (
        <p className="world-empty-state">No outputs</p>
      ) : (
        state.outputs.map((buffer) => (
          <div className="world-buffer" key={buffer.resourceId}>
            <div>
              <span>{resourceName(buffer.resourceId)}</span>
              <b>
            ...
            <Meter value={percent(buffer.quantity, buffer.capacity)} />
            <small>
              Work in progress <b>{buffer.wipPercent.toFixed(1)}%</b>
            </small>
            <Meter value={buffer.wipPercent} className="is-wip" />
          </div>
        ))
...
const MineState = ({
  ...
  readonly state: Extract<WorldBuildingSnapshot, { kind: 'mine' }>;
}) => (
  <>
    <div className="world-buffer-group">
      ...
      <div className="world-buffer">
        <div>
          <span>{resourceName(state.output.resourceId)}</span>
          <b>
            {state.output.quantity} / {state.output.capacity}
          </b>
        </div>
        <Meter value={percent(state.output.quantity, state.output.capacity)} />
      </div>
    </div>
    <dl className="world-building-counts">
      ...
      <div>
        <dt>Exhausted drills</dt>
        <dd>{state.drills.exhausted}</dd>
      </div>
    </dl>
  ...
  stations.length === 0 ? null : (
    <div className="world-logistics-state">
      ...
      {stations.map((station) => (
        <div className="world-logistics-row" key={station.id}>
          <div>
            <span>
...
export const WorldBuildingInspector = ({
  ...
  return (
    <>
      ...
      {state?.kind === 'storage' && (
        <Inventory title="Shared inventory" inventory={state.inventory} />
      )}
      ...
      {entity.kind === 'construction-site' && (
        <Inventory
          ...
          inventory={{
            capacity: entity.required.reduce(
              (sum, item) => sum + item.quantity,
              0,
            ),
            items: entity.delivered,
          }}
        />
      )}
      {entity.kind === 'drill' && (
