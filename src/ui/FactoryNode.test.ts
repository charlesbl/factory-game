...
describe('factory node port rates', () => {
  ...
  it('distinguishes the current recipe flow from the port capacity', () => {
    expect(formatPortRate(parseRate('2'), parseRate('4'))).toBe(
      '2/s (max 4/s)',
    );
    expect(formatPortRate(parseRate('1'), parseRate('2'))).toBe(
      '1/s (max 2/s)',
    );
  });
  ...
  it('measures machines against the craft rate instead of connector capacity', () => {
    ...
    expect(
      portUtilizationPercent(
        parseRate('2'),
        portMeterLimit(craftRate, connectorCapacity),
      ),
    ).toBe(50);
  });
  ...
  it('keeps connector capacity as the meter limit when there is no craft rate', () => {
    expect(
      portUtilizationPercent(
        parseRate('2'),
        portMeterLimit(undefined, parseRate('6')),
      ),
    ).toBe(33);
  });
  ...
  it('uses the pinned child contract rates for a sub-factory port', () => {
    ...
    const node: SubFactoryNode = {
      ...
      kind: 'sub-factory',
      name: 'Smelter',
      factoryId: asId('smelter'),
      version: 1,
      contractId: asId('contract'),
      position: { x: 0, y: 0 },
      footprint: { x: 0, y: 0, width: 4, height: 4 },
      ports: [
        {
          id: asId('input'),
          contractPortId: asId('contract-input'),
          direction: 'input',
          resourceId,
          capacity: parseRate('8'),
          anchor: { x: 0, y: 1 },
        },
      ],
    };
    const childContract: FactoryContract = {
      schemaVersion: 1,
      blueprintHash: 'contract',
      inputRates: new Map([[resourceId, parseRate('2')]]),
      outputRates: new Map(),
      inputPorts: [],
      outputPorts: [],
  ...
  it('uses each child contract port rate when several ports carry the same resource', () => {
    ...
    const ports = rates.map((rate, index) => ({
      id: asId<PortId>(`input-${index}`),
      contractPortId: asId<PortId>(`contract-input-${index}`),
      direction: 'input' as const,
      resourceId,
      capacity: rate,
      anchor: { x: 0, y: index + 1 },
    }));
    const node: SubFactoryNode = {
      id: asId('sub-factory'),
      kind: 'sub-factory',
      ...
      contractId: asId('contract'),
      position: { x: 0, y: 0 },
      footprint: { x: 0, y: 0, width: 4, height: 5 },
      ports,
    };
    const childContract: FactoryContract = {
      schemaVersion: 1,
      blueprintHash: 'contract',
      inputRates: new Map([[resourceId, parseRate('16')]]),
      outputRates: new Map(),
      inputPorts: ports.map((port, index) => ({
        portId: port.contractPortId,
        resourceId,
    ...
    expect(
      ports.map((_, index) => nominalPortRate(node, index, childContract)),
    ).toEqual(rates);
  });
  ...
  it('does not invent per-port rates when a legacy contract only has an aggregate', () => {
    ...
    const ports = [0, 1, 2].map((index) => ({
      ...
      direction: 'input' as const,
      resourceId,
      capacity: parseRate('6'),
      anchor: { x: 0, y: index + 1 },
    }));
    const node: SubFactoryNode = {
      id: asId('legacy-sub-factory'),
      kind: 'sub-factory',
      name: 'Smelter',
      factoryId: asId('smelter'),
      version: 1,
      contractId: asId('legacy-contract'),
      position: { x: 0, y: 0 },
      footprint: { x: 0, y: 0, width: 4, height: 5 },
      ports,
    };
