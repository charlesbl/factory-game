...
const App = () => {
  ...
  const openFactory = async (factoryId: FactoryId, nextView: 'factory' | 'world' = 'factory') => {
    ...
    try {
      ...
      localStorage.setItem(LAST_OPENED_FACTORY_KEY, factoryId); setActiveFactoryId(factoryId); setBlueprint(next); setCompilationBlueprint(next); setContract(undefined); setDiagnostics([]); setSelection(emptySelection()); setCompileState('compiling'); setView(nextView); setLibraryError(undefined)
  ...
  const createFactory = async (name: string) => {
    ...
    try {
      ...
      const next = history.replace(empty); localStorage.setItem(LAST_OPENED_FACTORY_KEY, id); setActiveFactoryId(id); setBlueprint(next); setCompilationBlueprint(next); setSelection(emptySelection()); setCompileState('compiling'); setView('factory')
  ...
  const forkVersion = async (version: FactoryVersion, name: string) => {
    ...
    try {
      ...
      setDefinitions((current) => [...current, definition].sort((a, b) => a.name.localeCompare(b.name))); setDrafts((current) => [...current, draft])
      const next = history.replace(forkedBlueprint); localStorage.setItem(LAST_OPENED_FACTORY_KEY, id); setActiveFactoryId(id); setBlueprint(next); setCompilationBlueprint(next); setSelection(emptySelection()); setCompileState('compiling'); setView('factory'); setLibraryError(undefined)
  ...
  const revertDraft = async (factoryId: FactoryId) => {
    ...
    try {
      ...
      if (factoryId === activeFactoryId) {
        ...
        setBlueprint(next); setCompilationBlueprint(next); setSelection(emptySelection()); setCompileState('compiling')
      }
    ...
    try {
      ...
      if (factoryId === activeFactoryId) {
        ...
        if (nextDefinition === undefined) {
          ...
          localStorage.setItem(LAST_OPENED_FACTORY_KEY, nextDefinition.id); setActiveFactoryId(nextDefinition.id); setBlueprint(next); setCompilationBlueprint(next); setSelection(emptySelection()); setCompileState('compiling'); setView('library')
        }
      }
  ...
  const deleteSelection = () => {
    ...
    const independentLooseConnections = selection.looseConnectionIds.filter((id) => {
  ...
  const copySelection = () => {
    const payload = captureSubgraph(blueprint, selection.nodeIds, selection.looseConnectionIds)
  ...
  const cutSelection = () => {
    ...
    const selectedNodes = new Set(selection.nodeIds)
    const looseConnections = selection.looseConnectionIds.filter((id) => { const loose = blueprint.looseConnections.get(id); return loose !== undefined && !selectedNodes.has(loose.origin.nodeId) })
    setClipboard(payload); execute(transaction('Cut selection', [...selection.nodeIds.map(removeNode), ...looseConnections.map(removeLooseConnection)])); setSelection(emptySelection()); setPlacement(undefined); setLibraryError(undefined)
  }
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (modified && key === 'c' && (selection.nodeIds.length > 0 || selection.looseConnectionIds.length > 0)) { event.preventDefault(); copySelection(); return }
      if (modified && key === 'x' && (selection.nodeIds.length > 0 || selection.looseConnectionIds.length > 0)) { event.preventDefault(); cutSelection(); return }
      if (modified && key === 'v' && clipboard !== undefined) { event.preventDefault(); pasteSelection(); return }
      if (modified && key === 'd' && (selection.nodeIds.length > 0 || selection.looseConnectionIds.length > 0)) { event.preventDefault(); duplicateSelection(); return }
      if (!modified && (event.key === 'Delete' || event.key === 'Backspace') && (selection.nodeIds.length > 0 || selection.edgeIds.length > 0 || selection.looseConnectionIds.length > 0)) { event.preventDefault(); deleteSelection() }
    }
  ...
  const selectedLooseConnection = selection.looseConnectionIds.length === 1 && selection.nodeIds.length === 0 && selection.edgeIds.length === 0 ? blueprint.looseConnections.get(selection.looseConnectionIds[0]!) : undefined
  ...
  const changeSubFactoryVersion = (target: FactoryVersion) => {
    ...
    const selectedId = selectedNode.id; execute(replaceSubFactoryVersion(selectedId, replacement)); window.setTimeout(() => setSelection({ nodeIds: [selectedId], edgeIds: [], looseConnectionIds: [] }), 0); setLibraryError(undefined)
  }
  ...
  return <main className="app-shell">
    ...
    {view === 'factory' && activeDefinition !== undefined ? <div className="workspace">
      ...
      <aside className="inspector panel">
        <div className="panel-heading"><div><span className="eyebrow">Inspect</span><h2>{selectedNode?.kind === 'sub-factory' ? definitions.find((item) => item.id === selectedNode.factoryId)?.name ?? selectedNode.name : selectedNode?.name ?? (selectedEdge !== undefined ? 'Selected route' : selectedLooseConnection !== undefined ? 'Incomplete route' : 'Factory contract')}</h2></div><span>⌘</span></div>
