...
import type { EdgeId, GridPoint, GridRect, ItemQuantity, NodeId, PortId, ResourceId } from '../domain'
...
export interface SerializedFactoryContract {
  ...
  readonly diagnostics: readonly CompileDiagnostic[]
  readonly billOfMaterials?: readonly { readonly resourceId: string; readonly quantity: number }[]
}
export const serializeContract = (contract: FactoryContract): SerializedFactoryContract => ({
  ...
  billOfMaterials: (contract.billOfMaterials ?? []).map((item) => ({ resourceId: item.resourceId, quantity: item.quantity })),
})
...
export const deserializeContract = (contract: SerializedFactoryContract): FactoryContract => ({
  ...
  billOfMaterials: (contract.billOfMaterials ?? []).map((item) => ({ resourceId: asId<ResourceId>(item.resourceId), quantity: item.quantity })),
})
