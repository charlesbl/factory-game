...
describe('compiled footprint', () => {
  it('projects ports onto stable, non-overlapping perimeter cells', () => {
    ...
    const ports = projectExternalPorts(blueprint, compiled.footprint);
    const cells = ports.map((port) => `${port.position.x}:${port.position.y}`);
    expect(new Set(cells).size).toBe(cells.length);
    expect(
      ports[0]?.position.x === compiled.footprint.x ||
        ports[0]?.position.y === compiled.footprint.y,
    ).toBe(true);
  ...
  it('restores exact per-port rates in legacy contracts from their saved blueprint flows', () => {
    ...
    const legacy = {
      ...compiled,
      inputPorts: compiled.inputPorts.map((port) => ({
        portId: port.portId,
        resourceId: port.resourceId,
        capacity: port.capacity,
        position: port.position,
      })),
      outputPorts: compiled.outputPorts.map((port) => ({
        portId: port.portId,
        resourceId: port.resourceId,
        capacity: port.capacity,
        position: port.position,
