...
export const validateBlueprint = (blueprint: FactoryBlueprint): ValidationResult => {
  ...
  for (const edge of [...blueprint.edges.values()].sort((a, b) => a.id.localeCompare(b.id))) {
    ...
    if ((sourceResource !== undefined && sourceResource !== edge.resourceId) || (targetResource !== undefined && targetResource !== edge.resourceId)) diagnostics.push({ code: 'RESOURCE_MISMATCH', severity: 'error', entity: { edgeId: edge.id } })
    if (edge.capacity <= 0n) diagnostics.push({ code: 'NON_POSITIVE_CAPACITY', severity: 'error', entity: { edgeId: edge.id } })
    try { if (polylineLength(effectiveEdgePoints(blueprint, edge)) === 0) diagnostics.push({ code: 'ZERO_LENGTH', severity: 'error', entity: { edgeId: edge.id } }) } catch { diagnostics.push({ code: 'ZERO_LENGTH', severity: 'error', entity: { edgeId: edge.id } }) }
