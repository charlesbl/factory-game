...
export const shortestPaths = (
...
): ReadonlyMap<NodeId, GraphPath> => {
  const best = new Map<NodeId, GraphPath>();
  const queue: QueueValue[] = [
    { nodeId: sourceNodeId, distance: 0, edgeIds: [], nodeIds: [sourceNodeId] },
  ];
  while (queue.length > 0) {
    ...
    const edges = [...blueprint.edges.values()]
      ...
      .sort(
        (a, b) =>
          routePhysicalLength(blueprint, a) -
            routePhysicalLength(blueprint, b) || a.id.localeCompare(b.id),
      );
    for (const edge of edges)
      queue.push({
        nodeId: edge.targetNodeId,
        distance: current.distance + routePhysicalLength(blueprint, edge),
