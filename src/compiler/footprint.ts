...
export const projectExternalPorts = (blueprint: FactoryBlueprint, footprint: GridRect, edgeFlows?: ReadonlyMap<EdgeId, RateRaw>): readonly ProjectedBoundaryPort[] => {
  ...
  return [...blueprint.externalPorts].sort((a, b) => a.side.localeCompare(b.side) || a.offset - b.offset || a.portId.localeCompare(b.portId)).flatMap((external) => {
    ...
    const rate = edgeFlows === undefined ? undefined : [...blueprint.edges.values()].filter((edge) => direction === 'input' ? edge.sourceNodeId === node.id && edge.sourcePortId === port.id : edge.targetNodeId === node.id && edge.targetPortId === port.id).reduce((total, edge) => total + (edgeFlows.get(edge.id) ?? 0n), 0n)
    return [{ nodeId: node.id, direction, side: external.side, portId: port.id, resourceId: port.resourceId, capacity: port.capacity, position: positions[external.side], ...(rate === undefined ? {} : { rate }) }]
  })
}

export const hydrateBoundaryPortRates = (blueprint: FactoryBlueprint, contract: FactoryContract): FactoryContract => {
  if ([...contract.inputPorts, ...contract.outputPorts].every((port) => port.rate !== undefined)) return contract
  const ports = projectExternalPorts(blueprint, contract.footprint, contract.edgeFlows)
  return { ...contract, inputPorts: ports.filter((port) => port.direction === 'input'), outputPorts: ports.filter((port) => port.direction === 'output') }
}
