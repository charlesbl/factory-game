...
export const defaultWorldGenerationConfig = (
  seed = 'starter-world',
...
export interface StationWorldEntity extends EntityBase {
  readonly kind: 'station';
  readonly stationId: StationId;
  readonly linkedEntityId?: WorldEntityId;
  readonly railNodeId: RailNodeId;
}
export interface StorageWorldEntity extends EntityBase {
...
export interface ConstructionSiteWorldEntity extends EntityBase {
  ...
  readonly siteId: ConstructionSiteId;
  ...
  readonly state: 'WAITING' | 'READY' | 'CANCELLED' | 'EVACUATING';
}
...
export interface WorldRailNode {
  readonly id: RailNodeId;
  readonly position: GridPoint;
  readonly kind: 'endpoint' | 'junction' | 'station' | 'depot';
}
export interface WorldRailEdge {
  readonly id: RailEdgeId;
  readonly from: RailNodeId;
  readonly to: RailNodeId;
  readonly points: readonly GridPoint[];
  readonly length: number;
}
export interface WorldRailBlock {
  readonly id: RailBlockId;
  readonly edgeId: RailEdgeId;
  readonly occupantId?: PodId;
...
export type PodState =
  ...
  | 'WAITING_BERTH'
  | 'GRIDLOCKED';
export interface WorldPod {
  readonly id: PodId;
  readonly capacity: 10;
  readonly state: PodState;
  readonly nodeId: RailNodeId;
  readonly cargo?: WorldItemStack;
  readonly missionId?: DeliveryId;
  readonly motion?: PodMotion;
}
export interface PodMission {
  readonly id: DeliveryId;
  readonly providerId: StationId;
  readonly requesterId: StationId;
  readonly podId: PodId;
  readonly resourceId: ResourceId;
  readonly quantity: number;
  readonly routeEdgeIds: readonly RailEdgeId[];
  readonly createdAt: SimTime;
}
export interface WorldDiagnostic {
  readonly code: 'GRIDLOCK' | 'DESTINATION_BLOCKED' | 'NO_ROUTE' | 'NO_STORAGE';
...
export interface WorldStationSnapshot {
  ...
  readonly target: number;
  readonly priority: number;
  readonly berthPodId?: PodId;
}

export interface WorldBufferSnapshot extends WorldItemStack {
  readonly capacity: number;
}
export interface WorldFactoryBufferSnapshot extends WorldBufferSnapshot {
  readonly wipPercent: number;
}
export type WorldBuildingSnapshot =
  | {
      ...
      readonly state:
        ...
        | 'INVALID'
        | 'MAINTENANCE';
      readonly inputs: readonly WorldFactoryBufferSnapshot[];
      readonly outputs: readonly WorldFactoryBufferSnapshot[];
      readonly nextEventAt?: SimTime;
    }
  | {
      readonly entityId: WorldEntityId;
      readonly kind: 'storage';
  ...
  | {
      readonly entityId: WorldEntityId;
      readonly kind: 'mine';
      readonly output: WorldBufferSnapshot;
      readonly construction: WorldInventory;
      readonly salvage: WorldInventory;
      readonly drills: Readonly<
        Record<'ghost' | 'active' | 'exhausted', number>
      >;
    }
  | {
      readonly entityId: WorldEntityId;
...
export const rotatedSize = (
  ...
  if (
    ...
    throw new RangeError(
      'World footprint size must use positive safe integers',
    );
  if (rotation !== 0 && rotation !== 1 && rotation !== 2 && rotation !== 3)
    throw new RangeError('World rotation must be a quarter turn');
