...
export const deserializeWorldRuntime = (
  ...
  if (
    ...
    throw new Error('Unsupported or corrupt world state: expected schema 2');
  ...
  for (const edge of state.railEdges)
    runtime.addRailEdge({
      ...
      from: asId(edge.from),
      to: asId(edge.to),
      points: edge.points.map((point: GridPoint) =>
        gridPoint(point.x, point.y),
      ),
    });
  for (const saved of state.railBlocks) {
    const block = runtime.rails.blocks.get(saved.id);
    if (block === undefined || block.edgeId !== saved.edgeId)
  ...
  runtime.restoreEntities(
    state.entities.map(
      (entity) =>
        ({ ...entity, id: asId<WorldEntityId>(entity.id) }) as WorldEntity,
    ),
  ...
  runtime.restoreTraffic(state.traffic);
