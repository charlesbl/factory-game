  ...
  void (async () => {
    ...
    try {
      ...
      const childContracts = new Map(
        request.childContracts.map((contract) => [
      ...
      const contract = serializeContract({ ...compiled, blueprintHash });
      contracts.set(blueprintHash, contract);
      while (contracts.size > 64)
        contracts.delete(contracts.keys().next().value as string);
      self.postMessage({
        protocolVersion: 1,
        requestId: request.requestId,
        revision: request.revision,
        ...
        diagnostics: [
          {
            ...
            entity: {},
            details: {
              value: error instanceof Error ? error.message : 'Worker failure',
            },
          },
        ],
      } satisfies CompileResponse);
