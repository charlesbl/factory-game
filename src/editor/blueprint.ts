...
export interface FactoryVersionRef {
  readonly factoryId: FactoryId;
  readonly version: number;
}

export type BlueprintNodeKind =
  'machine' | 'junction' | 'external-input' | 'external-output' | 'sub-factory';
interface PortBase {
  readonly id: PortId;
...
export interface BlueprintPort extends PortBase {
  readonly resourceId: ResourceId;
}
export interface SubFactoryPort extends BlueprintPort {
  readonly contractPortId: PortId;
}
export interface JunctionPort extends PortBase {
...
export interface MachineNode extends BaseNode<BlueprintPort> {
  readonly kind: 'machine';
  readonly recipeId: RecipeId;
}
export interface JunctionNode extends BaseNode<JunctionPort> {
  readonly kind: 'junction';
}
export interface BoundaryNode extends BaseNode<BlueprintPort> {
  readonly kind: 'external-input' | 'external-output';
}
export interface SubFactoryNode extends BaseNode<SubFactoryPort> {
  readonly kind: 'sub-factory';
  readonly factoryId: FactoryId;
  readonly version: number;
  readonly contractId: ContractId;
}
export type BlueprintNode =
  MachineNode | JunctionNode | BoundaryNode | SubFactoryNode;

export interface RouteHandle {
  readonly id: RouteHandleId;
  readonly position: GridPoint;
}
export interface RouteBridge {
  readonly id: RouteBridgeId;
  readonly position: GridPoint;
}
...
export const findPort = (
  ...
  nodeId: NodeId,
  portId: PortId,
): AnyBlueprintPort | undefined =>
  blueprint.nodes.get(nodeId)?.ports.find((port) => port.id === portId);

export const absolutePortPosition = (
  blueprint: FactoryBlueprint,
  ...
  return node === undefined || port === undefined
    ? undefined
    : {
        x: node.position.x + port.anchor.x,
        y: node.position.y + port.anchor.y,
      };
};

export const occupiedPortIds = (
  blueprint: FactoryBlueprint,
  exceptLooseId?: LooseConnectionId,
): ReadonlySet<PortId> => {
  const occupied = new Set<PortId>();
  for (const edge of blueprint.edges.values()) {
    occupied.add(edge.sourcePortId);
    occupied.add(edge.targetPortId);
  }
  for (const loose of blueprint.looseConnections.values())
    if (loose.id !== exceptLooseId) occupied.add(loose.origin.portId);
  return occupied;
};
export const isPortOccupied = (
  ...
  portId: PortId,
  exceptLooseId?: LooseConnectionId,
): boolean => occupiedPortIds(blueprint, exceptLooseId).has(portId);

const junctionComponent = (
  blueprint: FactoryBlueprint,
  startId: NodeId,
): ReadonlySet<NodeId> => {
  const start = blueprint.nodes.get(startId);
  if (start?.kind !== 'junction') return new Set();
  const component = new Set<NodeId>([startId]);
  ...
  while (pending.length > 0) {
    ...
    for (const edge of blueprint.edges.values()) {
      ...
      if (
        other === undefined ||
        component.has(other) ||
        blueprint.nodes.get(other)?.kind !== 'junction'
      )
        continue;
      component.add(other);
      pending.push(other);
...
export const junctionResources = (
  blueprint: FactoryBlueprint,
  nodeId: NodeId,
): ReadonlySet<ResourceId> => {
  const component = junctionComponent(blueprint, nodeId);
  const resources = new Set<ResourceId>();
  for (const edge of blueprint.edges.values()) {
    const endpoints: readonly [NodeId, PortId][] = [
    ...
    for (const [id, portId] of endpoints) {
      ...
      const port = findPort(blueprint, id, portId);
      if (
        node?.kind !== 'junction' &&
        port !== undefined &&
        'resourceId' in port
      )
        resources.add(port.resourceId);
    }
  }
  return resources;
};

export const effectivePortResource = (
  blueprint: FactoryBlueprint,
  nodeId: NodeId,
  portId: PortId,
): ResourceId | undefined => {
  const node = blueprint.nodes.get(nodeId);
  const port = findPort(blueprint, nodeId, portId);
