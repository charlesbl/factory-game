...
export class BlueprintHistory {
  ...
  constructor(initial: FactoryBlueprint, limit = 100) {
    this.#current = initial;
    this.#limit = limit;
  }
  get current(): FactoryBlueprint {
    return this.#current;
  }
  get canUndo(): boolean {
  ...
  replace(blueprint: FactoryBlueprint): FactoryBlueprint {
    this.#past = [];
    this.#future = [];
