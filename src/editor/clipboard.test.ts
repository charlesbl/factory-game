...
describe('graph clipboard', () => {
  ...
  it('remaps every placed identity and translates saved route geometry', () => {
    const blueprint = createDemoBlueprint();
    const payload = captureSubgraph(blueprint, [...blueprint.nodes.keys()])!;
    const placed = materializeSubgraph(
      payload,
      gridPoint(30, 40),
      createIdFactory(),
    );
    const originalIds = new Set([
      ...
      ...payload.edges.flatMap((edge) => [
        edge.id,
        ...edge.routeHandles.map((handle) => handle.id),
    ...
    expect(
      placed.edges.every(
        (edge) =>
          placed.nodeIds.includes(edge.sourceNodeId) &&
          placed.nodeIds.includes(edge.targetNodeId),
      ),
    ).toBe(true);
    expect(
      placed.externalPorts.every(
        (external) =>
          placed.nodeIds.includes(external.nodeId) &&
          placed.nodes.some((node) =>
            node.ports.some((port) => port.id === external.portId),
          ),
      ),
    ).toBe(true);
  });
  ...
  it('preserves immutable sub-factory contract references', () => {
    ...
    const node: SubFactoryNode = {
      id: asId<NodeId>('child'),
      kind: 'sub-factory',
      name: 'Child',
      factoryId: asId<FactoryId>('factory-child'),
      version: 3,
      contractId: asId<ContractId>('contract-child-v3'),
      position: gridPoint(4, 6),
      footprint: { x: 0, y: 0, width: 4, height: 3 },
      ports: [
        {
          id: asId<PortId>('placed-input'),
          contractPortId,
          direction: 'input',
  ...
  it('copies an incomplete route with its origin and remaps its identities', () => {
    ...
    const loose = {
      ...
      routeHandles: [
        { id: asId<RouteHandleId>('loose-end'), position: gridPoint(12, 10) },
      ],
    };
    const source = {
      ...createBlueprint(asId<FactoryId>('factory-source')),
      nodes: new Map([[node.id, node]]),
      looseConnections: new Map([[loose.id, loose]]),
    };
    ...
    expect(placed.looseConnections[0]!.origin.portId).toBe(
      placed.nodes[0]!.ports[0]!.id,
    );
    expect(placed.looseConnections[0]!.routeHandles[0]!.position).toEqual(
      gridPoint(28, 34),
    );

    const inserted = new BlueprintHistory(
      createBlueprint(asId<FactoryId>('factory-target')),
    ).execute(insertSubgraph(placed));
