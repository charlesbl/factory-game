...
const translatePoint = (point: GridPoint, offset: GridPoint): GridPoint => ({
  x: point.x + offset.x,
  y: point.y + offset.y,
});
  ...
  return {
    nodes: nodes.map(
      (node) =>
        ({
          ...node,
          position: translatePoint(node.position, relative),
          ports: node.ports.map((port) => ({ ...port })),
        }) as BlueprintNode,
    ),
    edges: edges.map((edge) => ({
      ...edge,
      routeHandles: edge.routeHandles.map((handle) => ({
        ...handle,
        position: translatePoint(handle.position, relative),
      })),
      bridges: edge.bridges.map((bridge) => ({
        ...bridge,
        position: translatePoint(bridge.position, relative),
      })),
    })),
    looseConnections: looseConnections.map((loose) => ({
      ...loose,
      origin: { ...loose.origin },
      routeHandles: loose.routeHandles.map((handle) => ({
        ...handle,
        position: translatePoint(handle.position, relative),
      })),
    })),
...
export const captureSubgraph = (
  ...
  const selectedLoose = [...blueprint.looseConnections.values()].filter(
    (loose) =>
      explicitlySelectedNodes.has(loose.origin.nodeId) ||
      explicitlySelectedLoose.has(loose.id),
  );
  const selected = new Set([
    ...nodeIds,
    ...selectedLoose
      .filter((loose) => explicitlySelectedLoose.has(loose.id))
      .map((loose) => loose.origin.nodeId),
  ]);
  const nodes = [...selected].flatMap((id) => {
    const node = blueprint.nodes.get(id);
    return node === undefined ? [] : [node];
  });
  const edges = [...blueprint.edges.values()].filter(
    (edge) =>
      selected.has(edge.sourceNodeId) && selected.has(edge.targetNodeId),
  );
  const externalPorts = blueprint.externalPorts.filter((port) =>
    selected.has(port.nodeId),
  );
  return createClipboardPayload(nodes, edges, externalPorts, selectedLoose);
};

export const materializeSubgraph = (
  ...
  for (const node of payload.nodes) {
    ...
    for (const port of node.ports)
      portIds.set(port.id, idFactory.next('PortId'));
  }
  ...
  const edges = payload.edges.map((edge): BlueprintEdge => ({
    ...
    routeHandles: edge.routeHandles.map((handle) => ({
      id: idFactory.next('RouteHandleId'),
      position: translatePoint(handle.position, anchor),
    })),
    bridges: edge.bridges.map((bridge) => ({
      id: idFactory.next('RouteBridgeId'),
      position: translatePoint(bridge.position, anchor),
    })),
  }));
  const looseConnections = payload.looseConnections.map(
    (loose): LooseConnection => ({
      id: idFactory.next('LooseConnectionId'),
      origin: {
        nodeId: nodeIds.get(loose.origin.nodeId)!,
        portId: portIds.get(loose.origin.portId)!,
      },
      routeHandles: loose.routeHandles.map((handle) => ({
        id: idFactory.next('RouteHandleId'),
        position: translatePoint(handle.position, anchor),
  ...
  const externalPorts = payload.externalPorts.map((port) => ({
    ...port,
    nodeId: nodeIds.get(port.nodeId)!,
    portId: portIds.get(port.portId)!,
  }));
  return {
    nodes,
    edges,
    ...
    looseConnectionIds: looseConnections.map((loose) => loose.id),
  };
};

export const translateSubgraph = (
  subgraph: MaterializedSubgraph,
  anchor: GridPoint,
): MaterializedSubgraph => ({
  ...subgraph,
  nodes: subgraph.nodes.map(
    (node) =>
      ({
        ...node,
        position: translatePoint(node.position, anchor),
...
const assertUnique = <Id extends string>(
  existing: ReadonlyMap<Id, unknown>,
  ids: readonly Id[],
  kind: string,
): void => {
...
export const insertSubgraph = (
  ...
  apply(blueprint) {
    ...
    for (const loose of subgraph.looseConnections)
      looseConnections.set(loose.id, loose);
    let candidate: FactoryBlueprint = {
      ...blueprint,
      nodes,
      edges,
      looseConnections,
      externalPorts: [...blueprint.externalPorts, ...subgraph.externalPorts],
    };
    ...
    for (const edge of candidate.edges.values()) {
      ...
      if (occupied.has(source.id) || occupied.has(target.id))
        throw new Error('A port can carry only one route');
      occupied.add(source.id);
      occupied.add(target.id);
      const sourceResource = effectivePortResource(
        candidate,
        edge.sourceNodeId,
        edge.sourcePortId,
      );
    ...
    for (const loose of candidate.looseConnections.values()) {
      if (
        findPort(candidate, loose.origin.nodeId, loose.origin.portId) ===
        undefined
      )
        throw new Error(
          'An incomplete route must reference a pasted component port',
        );
      if (occupied.has(loose.origin.portId))
        throw new Error('A port can carry only one route');
    ...
    for (const external of subgraph.externalPorts) {
      ...
      if (port === undefined)
        throw new Error(
          'An external port must reference a pasted component port',
        );
    }
    ...
    const childContracts = new Set(
      subgraph.nodes.flatMap((node) =>
        node.kind === 'sub-factory' ? [node.contractId] : [],
      ),
    );
    const next = updateBlueprint(blueprint, {
      nodes: candidate.nodes,
      edges: candidate.edges,
      looseConnections: candidate.looseConnections,
      externalPorts: candidate.externalPorts,
    });
    return {
      blueprint: next,
      changes: { resources, childContracts, structureChanged: true },
    };
  },
});

export const subgraphSelection = (
  subgraph: MaterializedSubgraph,
): {
  readonly nodeIds: readonly NodeId[];
  readonly edgeIds: readonly EdgeId[];
