...
describe('point-to-point route geometry', () => {
  it('materializes stable orthogonal spans from private handles', () => {
    const route = materializeHandleRoute(gridPoint(0, 0), [{ id: asId<RouteHandleId>('handle-a'), position: gridPoint(4, 3) }], 'horizontal')
    expect(route.points).toEqual([gridPoint(0, 0), gridPoint(4, 0), gridPoint(4, 3)])
  ...
  it('derives bridge sections and markers directly from a route', () => {
    ...
    expect(routeSections(bridged, bridged.edges.get(edge.id)!)).toEqual(expect.arrayContaining([expect.objectContaining({ layerId: 'bridge' })]))
    expect(routeBridgeMarkers(bridged, bridged.edges.get(edge.id)!)).toHaveLength(2)
