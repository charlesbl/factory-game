...
interface SerializedPort { readonly id: string; readonly direction: 'input' | 'output'; readonly resourceId?: string; readonly contractPortId?: string; readonly capacity: string; readonly anchor: { readonly x: number; readonly y: number } }
interface SerializedNode { readonly id: string; readonly kind: BlueprintNode['kind']; readonly name: string; readonly position: { readonly x: number; readonly y: number }; readonly footprint: { readonly x: number; readonly y: number; readonly width: number; readonly height: number }; readonly ports: readonly SerializedPort[]; readonly recipeId?: string; readonly contractId?: string; readonly factoryId?: string; readonly version?: number }
...
export interface SerializedBlueprint { readonly schemaVersion: 5; readonly id: string; readonly revision: number; readonly name?: string; readonly nodes: readonly SerializedNode[]; readonly edges: readonly SerializedEdge[]; readonly looseConnections: readonly SerializedLooseConnection[]; readonly externalPorts: readonly ExternalPort[] }
...
const serializeNode = (node: BlueprintNode): SerializedNode => ({
  ...
  ...(node.kind === 'machine' ? { recipeId: node.recipeId } : node.kind === 'sub-factory' ? { contractId: node.contractId, factoryId: node.factoryId, version: node.version } : {}),
})
export const serializeBlueprint = (blueprint: FactoryBlueprint): SerializedBlueprint => ({
  schemaVersion: 5, id: blueprint.id, revision: blueprint.revision,
...
export const canonicalCompilationInput = (blueprint: FactoryBlueprint): string => {
  ...
  const serialized = serializeBlueprint({ ...blueprint, looseConnections: new Map() })
  const compilationNodes = serialized.nodes.map((node) => ({
    kind: node.kind, position: node.position, footprint: node.footprint,
    ports: node.ports.map((port) => ({ direction: port.direction, ...(port.resourceId === undefined ? {} : { resourceId: port.resourceId }), capacity: port.capacity, anchor: port.anchor })),
    ...(node.recipeId === undefined ? {} : { recipeId: node.recipeId }),
    ...(node.contractId === undefined ? {} : { contractId: node.contractId }),
  }))
  const compilation = { nodes: compilationNodes, edges: serialized.edges, externalPorts: serialized.externalPorts }
