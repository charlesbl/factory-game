...
export const parseRate = (decimal: string): RateRaw => {
  ...
  if (match?.[1] === undefined)
    throw new NumericParseError(`Invalid rate: ${decimal}`);
...
export const compareExact = (a: bigint, b: bigint): -1 | 0 | 1 =>
  a < b ? -1 : a > b ? 1 : 0;
...
export const ceilingDivide = (
  numerator: bigint,
  denominator: bigint,
): bigint => {
  if (numerator < 0n || denominator <= 0n)
    throw new RangeError(
      'Ceiling division requires non-negative numerator and positive denominator',
    );
...
export const scaleRate = (rate: RateRaw, ratio: FixedRatio): RateRaw =>
  (rate * ratio) / RATE_SCALE;
