...
export const gridPoint = (x: number, y: number): GridPoint => ({
  x: integer(x, 'x'),
  y: integer(y, 'y'),
});
export const gridSize = (width: number, height: number): GridSize => {
  if (integer(width, 'width') <= 0 || integer(height, 'height') <= 0)
    throw new RangeError('Grid size must be positive');
  return { width, height };
};
export const manhattanDistance = (a: GridPoint, b: GridPoint): number =>
  Math.abs(a.x - b.x) + Math.abs(a.y - b.y);
export const euclideanDistance = (a: GridPoint, b: GridPoint): number =>
...
export const polylineLength = (line: Polyline): number => {
  ...
  return line
    .slice(1)
    .reduce(
      (total, point, index) => total + euclideanDistance(line[index]!, point),
      0,
    );
};
...
export const boundingRect = (
  ...
  return {
    x: minX,
    y: minY,
    width: Math.max(...xs) - minX + margin + 1,
    height: Math.max(...ys) - minY + margin + 1,
  };
};
